...
const TargetCursor = ({
  ...
  const cornersRef = useRef<NodeListOf<HTMLDivElement> | Element[] | null>(null);
  ...
  useEffect(() => {
    ...
    const moveHandler = (e: MouseEvent) => {
      ...
      const isOverCard = Boolean(target?.closest('.tilted-card-figure, .dot-only-cursor, [data-cursor="dot"]'));
      if (isOverCard) {
        if (cornersRef.current) {
          gsap.to(Array.from(cornersRef.current), { opacity: 0, duration: 0.15, overwrite: 'auto' });
        }
      } else if (!isActiveRef.current) {
        if (cornersRef.current) {
          gsap.to(Array.from(cornersRef.current), { opacity: 1, duration: 0.15, overwrite: 'auto' });
        }
      }
    };
